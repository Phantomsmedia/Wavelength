...
#include "f\common\f_briefing_rangers.sqf"
	if (f_var_debugMode == 1) then
	{
	player sideChat format ["DEBUG (briefing.sqf): Briefing for %1 slot selected.",_unitfaction];
	};
...
if (_unitfaction == "pmc_baf") exitwith {

#include "f\common\f_briefing_pmc.sqf"
...
_immString = format["%1",_unit];
_immArray = toArray _immString;
_immArray resize 7;
if (toString _immArray == "UnitVIE") then { //CHECK IF THE NAME IS OF A VIETNAM CDF UNIT
	#include "f\common\f_briefing_vietnam.sqf"
} else {
